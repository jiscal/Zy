...
class ExtSummarizer(nn.Module):
    ...
    def forward(self, src, segs, clss, mask_src, mask_cls):
        ...
        # ----------------------------------------------------------------------------------------
        # src 预训练src经过补pad的值 (batch_size,525)
        # segs 预训练segment经过补pad的值(batch_size,525)
        # clss 是 < CLS > 所在位置(batch_size,49)
        #mask_src 为mask的src的值(batch_size,525)
        # mask_cls 为真实的抽取出来句子的位置(batch_size,49)
        top_vec=self.bert(src,segs,mask_src)
        #经过bert的向量 batch_size,max_len,768
        print(top_vec.shape)
        #从top_vec中获取对应的clss的向量
        # clss_vec=top_vec[:,clss,:] 维度不对
        clss_vec=top_vec[torch.arange(top_vec.size(0)).unsqueeze(1),clss]
        print(clss_vec.shape)
        sent_scores = self.ext_layer(clss_vec, mask_cls)
