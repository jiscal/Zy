...
def _calc_final_dist(_enc_batch_extend_vocab, vocab_dists, attn_dists, p_gens, batch_oov_len, vocab_size, batch_size):
    ...
    """
    # Multiply vocab dists by p_gen and attention dists by (1-p_gen) #(batch_size,28693)
    vocab_dists = [p_gen * dist for (p_gen, dist) in zip(p_gens, vocab_dists)]
    attn_dists = [(1 - p_gen) * dist for (p_gen, dist) in zip(p_gens, attn_dists)] #(4,186)
    ...
    print(tf.shape(attn_dists_projected))
    ...
    # # pad_vocab_dists=[tf.pad(vocal_dist for vocal_dist in vocab_dists,[[0,0],[0,batch_oov_len]]  )]
    # for i,vocal_dist in enumerate(vocab_dists):
    #     vocal_dist=tf.pad(vocal_dist,[[0,0],[0,batch_oov_len]])
    #     final_dist=vocal_dist+attn_dists_projected[i]
