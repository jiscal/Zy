...
def masked_attention(enc_padding_mask, attn_dist):
    ...
    """
    # attn_dist(batch_size,max_length,1)
...
class BahdanauAttention(tf.keras.layers.Layer):
    ...
    def call(self, dec_hidden, enc_output, enc_pad_mask, use_coverage=False, prev_coverage=None):
        ...
        #enc_pad_mask((batch_size, enc_max_len)
        ...
        if use_coverage and prev_coverage is not None:
            ...
            # 计算注意力权重值(score shape == (batch_size, max_length, 1))
        ...
        # # 使用注意力权重*编码器输出作为返回值，将来会作为解码器的输入  attention_weights(batch_size, max_length, 1)
...
class Decoder(tf.keras.Model):
    ...
    def call(self, dec_inputs, dec_hidden, enc_output, enc_pad_mask, prev_coverage, use_coverage=True):
        ...
        #dec_output（batch_size,hidden_size),hidden_size(batch_size,hidden_size)
...
class Pointer(tf.keras.layers.Layer):
    ...
    def call(self, context_vector, dec_hidden, dec_inp):
        ...
        #dec_inp(batch_size, embedding_dim)
        p_gen=tf.nn.sigmoid(self.w_s_reduce(dec_hidden)+self.w_i_reduce(dec_inp)+self.w_c_reduce(context_vector))
        return p_gen
...
if __name__ == '__main__':
    ...
    #enc_output(batch_size, enc_max_len,enc_units*2)
    #enc_hidden(batch_size,enc_units*2)
