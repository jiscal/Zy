...
def batch_greedy_decode(model, batch_data, vocab, params):
    ...
    for t in range(params['max_dec_len']):
        ...
        for i,value in enumerate(predicted_ids):
            preidicts[i]+=vocab.id_to_word(value)
        # 上一步结果用于下一步预测
        dec_input = tf.expand_dims(predicted_ids, 1)+" "
        #
        # print(tf.shape(dec_input))
        # print(preidicts)
        ...
        for i in range(num_orig_hyps):
            ...
            for j in range(params['beam_size'] * 2):
                '''
                    ...
                    补全代码
                '''
                hyp=h.extend(top_k_ids[i,j].numpy(),
                         top_k_log_probs[i,j],
                         new_hidden,
                         attn_dist)
                all_hyps.append(hyp)
